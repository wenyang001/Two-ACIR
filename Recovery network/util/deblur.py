...
from util import imsave, imread_uint
'''
...
def deblur_kernel(img_L, k):
    iter_num = 15
    upperleft, denominator = get_uperleft_denominator(img_L, k)
    rhos, sigmas = get_rho_sigma(sigma=0.255/255., iter_num=iter_num)
    rhos = np.float32(rhos)
    z = img_L
    for i in range(iter_num):
        rho = rhos[i]
        z = np.real(np.fft.ifft2((upperleft + rho * np.fft.fft2(z, axes=(0, 1))) / (denominator + rho), axes=(0, 1)))
    imsave(z,  r'/media/liu/Data/Cityscape/test1K/1.png')
...
if __name__ == '__main__':
    ...
    data_path = r'/media/liu/Data/Cityscape/test1K/GTT_1/berlin_000000_000019_leftImg8bitT0.png'
    img_L = imread_uint(data_path, 3)
